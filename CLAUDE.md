...
| Framework | React 19 + Vite + TypeScript + React Router v7 (framework mode) | Owner already knows React. The perf-critical part is canvas, which is framework-agnostic. Router v7 gives pre-rendering so `/country/bulgaria` is crawlable. |
...
| Repo visibility | Private | Flip later. Never assume public. |

## Structure

```
prototype/         frozen reference build — do not develop here, only read
content/           hand-authored source of truth. THE MOAT. plain text.
scripts/           content/ + upstream datasets -> static/data/
src/lib/core/      subject-agnostic: scheduler, progress store, shared types
src/lib/map/       renderer, projections (mercator + globe), interaction
src/lib/subjects/  per-subject question generators and panels
src/lib/ui/        design tokens, shared components
src/routes/        /  /atlas  /country/[slug]  /study/[mode]
static/data/       generated, committed on purpose
```
...
- Generated data is **committed**, not built at deploy time. A content edit shows up as a
  diff in both `content/` and `static/data/`.
- `src/lib/core/` may not import from `src/lib/subjects/`. The dependency runs one way, so
  history can be added without touching the scheduler.
...
npm run build:content   # content/ -> static/data/
npm run typecheck
npm run test            # headless browser smoke test
```
...
- Branch protection is unavailable on GitHub Free for private repos, so this is convention,
  not enforcement. Treat it as enforcement anyway.
