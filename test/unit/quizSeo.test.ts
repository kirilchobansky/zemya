...
const capitals = { seoName: 'Capitals', seoTask: 'Type the capital of each highlighted country.' };
const flags = { seoName: 'Flags', seoTask: 'Type the country each flag belongs to.' };

describe('quizPageSeo', () => {
  it('builds the title from quiz, scope and size', () => {
    expect(quizPageSeo(capitals, 'africa', '30', 54).title).toBe('Africa Capitals Quiz — Top 30 Countries | Zemya');
    expect(quizPageSeo(flags, 'europe', 'all', 46).title).toBe('Europe Flags Quiz — All 46 Countries | Zemya');
    expect(quizPageSeo({ seoName: 'Map', seoTask: '' }, 'world', '20', 197).title).toBe(
      'World Map Quiz — Top 20 Countries | Zemya'
    );
  });
